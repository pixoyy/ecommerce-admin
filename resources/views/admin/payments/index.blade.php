...
@section('title', 'Konfirmasi Pembayaran — EssenseLuxe Admin')
@section('page_title', 'Konfirmasi Pembayaran')

@section('content')
    <div class="flex items-center justify-between mb-5">
        <div>
            <p class="text-sm text-slate-500">Konfirmasi pembayaran dari pelanggan.</p>
        </div>
    </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-slate-100">
                        ...
                        <th class="text-left px-5 py-4 font-semibold text-slate-600">Akun Tujuan</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($payments as $payment)
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            <td class="px-5 py-4 text-slate-500">{{ $payments->firstItem() + $loop->index }}</td>
                            <td class="px-5 py-4">
                                <span class="font-mono text-xs font-medium text-slate-700">{{ $payment->order->order_number ?? '-' }}</span>
                            </td>
                            <td class="px-5 py-4 text-slate-600">
                                {{ $payment->paymentAccount?->bank_name }} — {{ $payment->paymentAccount?->account_number }}
                            </td>
                            ...
                            <td class="px-5 py-4 text-slate-500 text-xs">{{ $payment->created_at->format('d M Y H:i') }}</td>
                            <td class="px-5 py-4">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-5 py-12 text-center">
                                <i class="bi bi-credit-card-2-front text-4xl text-slate-300 block mb-3"></i>
